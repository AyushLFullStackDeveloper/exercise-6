...
class EnterpriseReporter {
  onRunComplete(contexts, results) {
    ...
    [htmlDir, junitDir, logsDir, summariesDir].forEach(ensureDir);
    ...
    fs.writeFileSync(
      path.join(summariesDir, 'failed-tests-summary.json'),
      JSON.stringify(
        {
    ...
    fs.writeFileSync(
      path.join(summariesDir, 'execution-summary.md'),
      [
        '# API Test Execution Summary',
        '',
        `Generated At: ${summary.generatedAt}`,
        '',
        `Total Suites: ${summary.numTotalTestSuites}`,
        `Passed Suites: ${summary.numPassedTestSuites}`,
        `Failed Suites: ${summary.numFailedTestSuites}`,
        `Total Tests: ${summary.numTotalTests}`,
        `Passed Tests: ${summary.numPassedTests}`,
        `Failed Tests: ${summary.numFailedTests}`,
        `Pending Tests: ${summary.numPendingTests}`,
      ].join('\n'),
    );

    fs.writeFileSync(
      path.join(summariesDir, 'coverage-summary.json'),
      JSON.stringify(
        {
          generatedAt: summary.generatedAt,
          coverageMapAvailable: Boolean(results.coverageMap),
          note: results.coverageMap
            ? 'Jest coverage data was available for this run.'
            : 'No Jest coverage map was produced for this run.',
        },
        null,
        2,
      ),
